...
public class WSContextFactory implements InitialContextFactory, ObjectFactory, AllServiceListener {
    ...
    private final ConcurrentHashMap<ServiceReference<?>, Object> cache = new ConcurrentHashMap<ServiceReference<?>, Object>();
    ...
    Object getService(final ServiceReference<?> ref) {
        Object val = cache.get(ref);
        if (val == null) {
            synchronized (cache) {
                val = cache.get(ref);
                if (val == null) {
                    val = AccessController.doPrivileged(new PrivilegedAction<Object>() {
                        @Override
                        public Object run() {
